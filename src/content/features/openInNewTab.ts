...
import {
  CHOOSE_MEAL_BUTTON_TEST_ID,
  EATER_OPTION_CARD_TEST_ID,
  ORDER_HUMAN_ID_TEST_ID,
} from '@content/elementNames.ts';
import {
  selectElementByTestId,
  selectElementsByTestId,
} from '@content/elementSelectors.ts';
...
const getCardsWithChooseButton = (cards: HTMLElement[]) =>
  cards.filter((card) => {
    const chooseButton = selectElementByTestId(
      CHOOSE_MEAL_BUTTON_TEST_ID,
      card,
    );
...
export const openInNewTab: Feature = {
  ...
  async run(): Promise<void> {
    ...
    logger.debug(
      { cards },
      `openInNewTab.run: found ${EATER_OPTION_CARD_TEST_ID} cards`,
    );

    const cardsWithChooseButton = getCardsWithChooseButton(cards);
    logger.debug(
      { cardsWithChooseButton },
      'openInNewTab.run: cards with choose button',
    );

    const cartCacheStorage = await cartCache.get();

    cardsWithChooseButton.forEach((x) =>
      updateCardButtonWithLink(x, cartCacheStorage),
    );
  },
};

const updateCardButtonWithLink = (
  card: HTMLElement,
  cartCacheStorage: CartCacheStorage,
) => {
  const humanIdElement = selectElementByTestId(ORDER_HUMAN_ID_TEST_ID, card);
  const rawHumanOrderIdText = humanIdElement?.textContent
    ?.trim()
    .replace('- Order ', '');
  ...
  if (!humanOrderId.success) {
    logger.warn(
      { rawText: rawHumanOrderIdText },
      'Could not find human ID for card, skipping',
    );
    return;
  }
  ...
  if (!id) {
    logger.warn(
      ...
      'Could not find order ID for human ID, skipping',
    );
    return;
  }
  ...
  if (!button) {
    logger.warn(
      { humanOrderId: humanOrderId.data },
      'Could not find choose button for card, skipping',
    );
    return;
  }
